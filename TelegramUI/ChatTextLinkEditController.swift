...
func chatTextLinkEditController(sharedContext: SharedAccountContext, account: Account, text: String, link: String?, apply: @escaping (String?) -> Void) -> AlertController {
    ...
    applyImpl = { [weak contentNode] in
        ...
        if !updatedLink.hasPrefix("http") && !updatedLink.hasPrefix("https") && !updatedLink.hasPrefix("tg") && !updatedLink.hasPrefix("ng") {
...
private final class NiceInputFieldNode: ASDisplayNode, ASEditableTextNodeDelegate {
    private var theme: PresentationTheme
    private let backgroundNode: ASImageNode
    private let textInputNode: EditableTextNode
    private let placeholderNode: ASTextNode
...
private final class NiceInputAlertContentNode: AlertContentNode {
    private let strings: PresentationStrings
    private let text: String
    
    private let titleNode: ASTextNode
    private let textNode: ASTextNode
    let inputFieldNode: NiceInputFieldNode
    ...
    init(theme: AlertControllerTheme, ptheme: PresentationTheme, strings: PresentationStrings, actions: [TextAlertAction], text: String, link: String?) {
        ...
        self.inputFieldNode = NiceInputFieldNode(theme: ptheme, placeholder: l(key: "Folder.Name", locale: strings.baseLanguageCode))
        self.inputFieldNode.text = link ?? ""
    ...
    override func updateTheme(_ theme: AlertControllerTheme) {
        self.titleNode.attributedText = NSAttributedString(string: l(key: "Folder.New", locale: self.strings.baseLanguageCode), font: Font.bold(17.0), textColor: theme.primaryColor, paragraphAlignment: .center)
        // self.textNode.attributedText = NSAttributedString(string: self.strings.TextFormat_AddLinkText(self.text).0, font: Font.regular(13.0), textColor: theme.primaryColor, paragraphAlignment: .center)
    ...
    override func updateLayout(size: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        // let textSize = self.textNode.measure(measureSize)
        // transition.updateFrame(node: self.textNode, frame: CGRect(origin: CGPoint(x: floorToScreenPixels((size.width - textSize.width) / 2.0), y: origin.y), size: textSize))
        origin.y += 6.0 // textSize.height + 6.0
...
func niceInputAlertController(sharedContext: SharedAccountContext, account: Account, text: String, link: String?, apply: @escaping (String?) -> Void) -> AlertController {
    let presentationData = sharedContext.currentPresentationData.with { $0 }
    ...
    let contentNode = NiceInputAlertContentNode(theme: AlertControllerTheme(presentationTheme: presentationData.theme), ptheme: presentationData.theme, strings: presentationData.strings, actions: actions, text: text, link: link)
    contentNode.complete = {
        applyImpl?()
    }
    applyImpl = { [weak contentNode] in
